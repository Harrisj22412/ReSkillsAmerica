...
#database = {
    #357229167: ['Jalen', 'Harris', 'harrisj22412@hocking.edu', 'PasswordJalen', 300]
# }
...
#def logout():
    #login(account_number, user_details)
...
def login(account_number, user_details):
    
    print('****** Login*********')

    try:
        f = open('data/auth_session' + str(account_number) + '.txt', 'x')
        f.write(str(user_details + current_time + 'This is when the user logged in'))
        ...
        f.write(str(user_details + current_time + d1 + 'This is when the user logged in for account number' + account_number))
        

    finally: 
        print('User logged out' + current_time)
